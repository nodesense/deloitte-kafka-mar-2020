...
public class StockConsumer {
    ...
    public static void main(String[] args) {
        ...
        props.put(GROUP_ID_CONFIG, "stock-consumer-example"); // offset, etc, TODO
        ...
        while (true) {
            ...
            // <Key, Value>
            ConsumerRecords<String, Stock> records = consumer.poll(ofSeconds(1));
            // if no messages
            if (records.count() == 0)
                continue;
            ...
            for (ConsumerRecord<String, Stock> record : records) {
                ...
                String o = (String) genericRecord.get("id");

                Integer qty = (Integer) genericRecord.get(1); // by index


                Stock stock = (Stock) SpecificData.get().deepCopy(Stock.SCHEMA$, genericRecord);

                System.out.println("Stock ID " + stock.getId());
                System.out.println("Stock Qty " + stock.getQty());
